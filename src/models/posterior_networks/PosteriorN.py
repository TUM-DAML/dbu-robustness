...
from src.models.posterior_networks.NormalizingFlowDensity import NormalizingFlowDensity
from src.models.posterior_networks.MixtureDensity import MixtureDensity

__architectures__ = {'linear': None,
                    'conv': None,
                    'vgg': None}
    ...
    def forward(self, input, soft_output=None, return_output='alpha', compute_loss=False):
        ...
        if self.no_density:
            ...
            for c in range(self.output_dim):
                ...
                log_q_zk[:, c] = log_p
                alpha[:, c] = 1. + (N[c] * torch.exp(log_q_zk[:, c]))  # shape: [batch_size]
            soft_output_pred = torch.nn.functional.normalize(alpha, p=1)  # shape: [batch_size, output_dim]
